...
import { Heart, Map, Users, AlertTriangle, Trophy, Menu, X, LogOut, User, Recycle, Store, LayoutDashboard } from "lucide-react";
...
export function Navbar() {
  ...
  const mainLinks = [
    ...
    { href: "/invendus", label: "Invendus", icon: <Recycle size={16} /> },
  ...
  const dashboardLink = user?.role === "commerce"
    ? { href: "/dashboard/commerce", label: "Mon espace", icon: <LayoutDashboard size={16} /> }
    : user?.role === "association"
    ? { href: "/dashboard/association", label: "Mon asso", icon: <LayoutDashboard size={16} /> }
    : null;
        ...
        <div className="hidden md:flex items-center gap-1">
          ...
          {dashboardLink && (
            <Link href={dashboardLink.href}
              className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm font-medium text-emerald-600 hover:bg-emerald-50 transition-colors bg-emerald-50/60">
          ...
          <div className="border-t border-gray-100 pt-2 mt-2">
            {user ? (
              <div className="flex items-center justify-between px-3 py-2">
                <Link href="/profil" onClick={() => setOpen(false)} className="text-sm text-gray-700">👤 {user.name}</Link>
                <button onClick={logout} className="flex items-center gap-1 text-sm text-red-500">
